...
class BrainDQN:
    ...
    def gen_duel_sym(self, n_acts, batch_size, predict=False):
        ...
        mean = sum / n_acts
        mean = mx.sym.Reshape(data=mean, shape=(batch_size, 1), name='rs_mean')

        sub_stream = mx.sym.broadcast_minus(adv_stream, mean)
        Qvalue = mx.sym.broadcast_plus(sub_stream, val_stream)
    ...
    def createQNetwork(self,bef_args=None,isTrain=True):
        if isTrain:
            batch=BATCH_SIZE
            if DUEL:
                modQ = mx.mod.Module(symbol=self.gen_duel_sym(self.actions, batch), data_names=('data','actionInput'), label_names=('yInput',), context=ctx)
            else:
            ...
            batch=1
            if DUEL:
                modQ = mx.mod.Module(symbol=self.gen_duel_sym(self.actions, batch, predict=True), data_names=('data',), label_names=None, context=ctx)
            else:
